...
```
해석적: dL/dw=-0.26526985862215685
수치적: dL/dw=-0.26526985864860910
차이:   2.65e-11
```

소수점 열 자리까지 같고, 열한 번째에서 갈린다. 이 대조를 **그래디언트 체크** 라고 부르고, 직접 층을 구현할 때
가장 먼저 붙여야 하는 안전장치다. 수치미분은 파라미터 하나당 순전파를 두 번
해야 해서 학습에는 못 쓴다. 검산용으로도 위에서 본 `h` 문제를 안고 있으니,
"몇 자리까지 맞아야 한다" 는 기준을 정해두고 쓰는 게 낫다.
