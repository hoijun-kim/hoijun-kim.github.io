...
4편과 같은 20층 그물에 이 한 줄만 넣고, 초기값 셋을 그대로 돌린다.
...
`BN(cz) = BN(z)` 이고, 실제로 `z` 와 `100z` 를 넣어 보면 차이가 `6e-15` 로 부동
소수점 잡음뿐이다. 초기값 0.01, 1.0, Xavier 는 같은 난수에 곱한 상수만 다르므로,
...
잘리고 첨도가 배치 4에서 `1.835`, 배치 512에서 `3.003` 으로 달라진다. `tanh` 는
