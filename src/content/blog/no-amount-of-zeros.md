...
**99%를 0으로 만들어도 시간이 그대로다.** 행렬곱은 여전히 촘촘한 행렬곱이고,
0을 곱하는 것도 곱하는 것이다. 건너뛰려면 어디가 0인지 확인해야 하는데, 그
확인이 곱셈보다 비싸다.
...
여덟 편이다. 고르는 규칙, 두 번 계산하지 않는 법, 정밀도를 버리는 값, 미리 써
보는 값, 한꺼번에 처리하는 값, 버릴 것을 고르는 값, 읽기와 쓰기의 값, 그리고
개수를 줄이는 값.
